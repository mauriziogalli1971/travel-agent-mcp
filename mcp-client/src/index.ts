	...
	async fetch(request: Request, env: ConfigOptions & Env) {
		...
		try {
			...
			if (!initResponse.ok) {
				...
				return new Response(
					JSON.stringify({ error: "Init failed", details: errorText }),
					{
			...
			try {
				...
				try {
					...
					logger.logInfo("request.start", "Calling MCP tool endpoint", {
						trip: tripData,
					});

					const openai = new OpenAIService(config.OPENAI_API_KEY, {
						baseURL: config.CF_AI_GATEWAY,
					}).get();
					...
					logger.logInfo?.(
						"request.start.openai",
						JSON.stringify(openai.config, null, 2),
					);
...
3) Hotel: best hotel in trip.to for trip.travellers people, check-in trip.start, check-out trip.end, max cost trip.budget.`,
					...
					const parsed = TripDataExtraction.safeParse(
					...
					logger.logInfo(
						"request.success",
						"`tripData` successfully retrieved",
						{ result: tripData },
					);
			...
			logger.logError("debug.tools.error", "Error in MCP Tool request", {
				error: e.message,
				stack: e.stack,
			});
