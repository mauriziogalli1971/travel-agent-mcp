import { Config, type ConfigOptions } from './infra/config/env';
import { OpenAIService } from './infra/ai/openaiService';
import { Logger, mapErrorToHttp, isLocalDev, TripData, type TripResponse, RemoteApiError } from '@travel-agent/shared';
import { z } from 'zod';
...
export default {
  async fetch(request: Request, env: ConfigOptions & Env) {
    ...
    try {
      ...
      try {
        ...
        try {
          ...
          logger.logInfo('request.start', 'Calling MCP tool endpoint', { trip: tripData });

          const openai = new OpenAIService(config.OPENAI_API_KEY, { baseURL: config.CF_AI_GATEWAY }).get();
          const TripDataExtraction = z.object({ weather: z.string(), flight: z.string(), hotel: z.string() });
            ...
            input: [
              {
                ...
                content: [
                  {
                    type: 'input_text',
                    text: `You are a travel assistant with access to tools via MCP.
...
- Hotel: best option. If no hotels: "I don't have any hotels available."`,
              ...
              },
              {
                ...
                content: [
                  ...
                  {
                    type: 'input_text',
                    text: `Tasks (use fields from the trip data):
...
3) Hotel: best hotel in trip.to for trip.travellers people, check-in trip.start, check-out trip.end, max cost trip.budget.`,
            ...
            text: { format: zodTextFormat(TripDataExtraction, 'trip_data_extraction') },
          });
          ...
          logger.logInfo('request.success', '`tripData` successfully retrieved', { result: tripData });
      ...
      logger.logError('debug.tools.error', 'Error in MCP Tool request', { error: e.message, stack: e.stack });
