...
export default function Settings(): JSX.Element {
  ...
  const [from, setFrom] = useState<string>('Trieste');
  const [to, setTo] = useState<string>('London');
  ...
  const minDateFormatted = formatDate(new Date(TOMORROW.getTime()));
  const maxDateFormatted = formatDate(new Date(MAX_DATE.getTime()));
  const [start, setStart] = useState<string>(minDateFormatted);
  const [end, setEnd] = useState<string>(formatDate(new Date(TOMORROW.getTime() + MIN_STAY)));
  ...
  useEffect(() => {
    navigator.geolocation.getCurrentPosition((position) => {
      ...
      getLocationName(userLocation)
        .then((location: { address: { city: string } }): void => {
  ...
  return !trip ? (
    <form
      className='view form'
      onSubmit={async (e: FormEvent<HTMLFormElement>): Promise<void | JSX.Element> => {
        e.preventDefault();
        loader.current?.classList.add('loading');
        try {
          const tripData = await callTravelAgentWorker();
          setTrip(tripData);
    ...
    >
      ...
      <div className='form-control-wrapper'>
        ...
        <div className='form-control'>
          ...
          <input id='travellers' name='travellers' type='text' readOnly value={travellers} />
      ...
      <div className='form-control-wrapper'>
        ...
        <div className='form-control'>
          <input id='to' name='to' type='text' value={to} onChange={(e) => setTo(e.target.value)} />
        </div>
      </div>
  ...
  async function callTravelAgentWorker(): Promise<TripData> {
    // @ts-ignore
